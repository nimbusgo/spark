...
def docker_setup(master_nodes, slave_nodes, opts):
    master_hostname = get_dns_name(master_nodes[0], opts.private_ips)
    spark_master = "spark://%s:7077" % master_hostname

    pull_image = "docker pull nimbusgo/spark"
    run_master = "docker run --net=host --name=master -dt nimbusgo/spark /usr/local/spark/sbin/start-master.sh"
    run_slave = 'docker run --net=host --name=slave -dt nimbusgo/spark /usr/local/spark/sbin/start-slave.sh %s' % spark_master

    reset_master = "docker kill master || true && docker rm master || true"
    reset_slave = "docker kill slave || true && docker rm slave || true"

    print("run slave: ", run_slave)
    all_nodes = master_nodes + slave_nodes

    all_hostnames = [get_dns_name(node, opts.private_ips) for node in all_nodes]
    multi_ssh(all_hostnames, opts, pull_image)

    ssh(master_hostname, opts, reset_master)
    ssh(master_hostname, opts, run_master)

    for node in slave_nodes:
        hostname = get_dns_name(node, opts.private_ips)
        ssh(hostname, opts, reset_slave)
        ssh(hostname, opts, run_slave)


# Deploy configuration files and run setup scripts on a newly launched
# or started EC2 cluster.
def setup_cluster(conn, master_nodes, slave_nodes, opts, deploy_ssh_key):
    docker_setup(master_nodes, slave_nodes, opts)
    print("Done!")
...
def multi_ssh(hosts, opts, command):
    threads = [threading.Thread(target= ssh(host, opts, command)) for host in hosts]
